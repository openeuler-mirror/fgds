...
Standalone GDS benchmark: torch.cuda.gds write/read for multiple rounds.

Run from outside the raw source tree, e.g.:
  cd /tmp && python /path/to/agent_space/bench_gds_pytorch.py
  cd /tmp && python /path/to/agent_space/bench_gds_pytorch.py 1   # use cuda:1
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if sys.platform == "win32":
        print("SKIP: GdsFile is not supported on Windows", file=sys.stderr)
        return 0

    device_id, num_rounds = parse_cuda_cli_args(argv, description="GDS read/write benchmark")
    ...
    try:
        ...
        fh = gds.GdsFile(path, os.O_CREAT | os.O_RDWR)
        try:
            gds.gds_register_buffer(src.untyped_storage())
            gds.gds_register_buffer(dest.untyped_storage())
            print(
            ...
            gds.gds_deregister_buffer(src.untyped_storage())
            gds.gds_deregister_buffer(dest.untyped_storage())
        ...
        if ok:
            print("PASS: gds round-trip read/write matches")
            return 0
