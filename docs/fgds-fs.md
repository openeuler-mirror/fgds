# Fgds-fs
The kernel module performs initialization and uninitialization operations when inserted and removed. We detail the initialization and uninitialization processes below, along with the character device interface provided by fgds-fs.
## 1. Kernel Module
### Initialization
```c
/** 
...
fgds-fs provides a character device interface that allows user-space applications to interact with the kernel module. The character device includes the following three basic operations:
...
int fgds_map_dev_addr(fgds_ioctl_map_t *map_param, u64 devaddr, u64 dev_len, u64 cpuvaddr, u64 length) {
    ...
    mbuffer = fgds_check_and_bind_mmap_buffer(cpuvaddr, length);
