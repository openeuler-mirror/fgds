...
class FgdsMemoryAllocator(GPUMemoryAllocator):
    def __init__(self, size: int, device=None):
        ...
        self.device = int(device_list.split(",")[get_device_id(device)])
...
class FgdsBackend(AllocatorBackendInterface):
    ...
    def __init__(
        ...
        device_idx = get_device_id(dst_device)
        device_list = os.environ.get("CUDA_VISIBLE_DEVICES", "0")
        ...
        self.use_fgds = True
        use_fgds_from_config = False

        if config.extra_config is not None:
            use_fgds = get_extra_config_bool("use_fgds", config)
            if use_fgds is not None:
                self.use_fgds = use_fgds
                use_fgds_from_config = True
        ...
        if self.fstype in ["tmpfs", "overlayfs"]:
            ...
            if use_fgds_from_config:
                logger.warning("No automatic disabling of fgds usage due to fstype")
            else:
        ...
        if self.use_fgds:
            ...
            self.cudart = None
            self.fgds = fgds_backend.fgds

            self._fgds_driver = self.fgds.FgdsDriver(self.device_id)
        else:
            logger.info("Not using fgds")
            self.fgds = None
