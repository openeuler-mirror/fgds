...
std::string block_file = "/mnt/fgds/data.bin";
...
FgdsKVCacheReader::FgdsKVCacheReader(size_t max_batch_size_, int device_id_)
                    : device_id(device_id_), max_batch_size(max_batch_size_) {
    int ret = fgds_open(this->device_id);
    if (ret) {
    ...
    for (size_t i = 0; i < max_batch_size; i++) {
        ...
        ret = fgds_regmem(this->device_id, devPtrs[i], MAX_BLOCKS_SIZE, &host_ptrs[i]);
        if (ret) {
            throw std::runtime_error("Buffer register failed");
        }
    }
...
void FgdsKVCacheReader::load_sequences(const std::string& trace_file) {
    std::ifstream file(trace_file);
    if (!file) {
        throw std::runtime_error("Cannot open trace file: " + trace_file);
    }
...
void FgdsKVCacheReader::process_all_sequences() {
...
FgdsKVCacheReader::~FgdsKVCacheReader() {
    io_uring_queue_exit(&ring);
    for (size_t i = 0; i < max_batch_size; i++) {
        fgds_deregmem(this->device_id, devPtrs[i], MAX_BLOCKS_SIZE);
        cudaFree(devPtrs[i]);
    }
    delete[] devPtrs;
    close(fd);
    fgds_close(this->device_id);
}

int main(int argc, char** argv) {
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " <type: fgds|gds> <gpu_id> <trace_file> <block_size>" << std::endl;
        return 1;
    }
