...
#define FGDS_CHECK_ERROR(status) { \
    if (status != 0) { \
      std::cerr << "(" << __func__ << ":" << __LINE__ << ")"; \
      std::cerr << "fgds error: " << status << "\n"; \
      exit(-1); \
    } \
} \
...
static inline uint64_t tensor_to_device_fgds(safetensors::safetensors_t &st, int fd){
  ...
  for (size_t idx = 0; idx < st.tensors.size(); idx++) {
    ...
    auto ret = fgds_regmem(device_id, tensor.dev_ptr, alloc_size, &host_ptr);
    if (ret){
      std::cerr << "fgds_regmem error, ret is " << ret << ", size is " << alloc_size << "\n";
      return -1;
    }

    ssize_t result = fgds_read({.fd = fd, .deviceID = device_id},
      tensor.dev_ptr, 0,
    ...
    FGDS_CHECK_ERROR(fgds_deregmem(device_id, tensor.dev_ptr, alloc_size));
  }
  return total_file_size;
}
  
static inline int read_tensor_fgds(std::string &filename, uint64_t *done_size) {
...
int load_safetensors_fgds(std::string &dir) {
    std::string warn, err;
    ...
    CUDA_CHECK_ERROR(cudaSetDevice(device_id));
    FGDS_CHECK_ERROR(fgds_open(device_id));
    ...
    for (auto &file : file_paths) {
      ...
      if (read_tensor_fgds(file, &done_size) != 0) {
    ...
    FGDS_CHECK_ERROR(fgds_close(device_id));
