...
static inline void loop_fgds_large(struct IOParams *params){
    struct timespec start, end;
    struct timespec io_start, io_end;
    int file_fd;
    int device_id = 0;
    ...
    ret = fgds_open(0);
    if (ret){
        printf("fgds open failed\n");
        exit(1);
    }
    
    ret = fgds_regmem(device_id, gpu_buffer, params->io_size, &target_addr);

    if (ret){
        printf("fgds regmem failed\n");
        exit(1);
    }

    clock_gettime(CLOCK_MONOTONIC, &io_start);
    result = fgds_read({.fd = file_fd, .deviceID = device_id}, gpu_buffer, 0, params->io_size, params->loop_idx * params->io_size);
    if (result != params->io_size){
        printf("fgds read failed\n");
        exit(1);
    }
    clock_gettime(CLOCK_MONOTONIC, &io_end);
...
int main(int argc, char* argv[]){
    ...
    std::cout << "Test mode: " << (mode == 0 ? "FGDS" : "GDS") << std::endl;
